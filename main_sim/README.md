...
> main_sim은 **기능 코드를 갖지 않는다.** 이동은 Nav2 액션, 추적은 `tracking` 패키지에
...
```bash
# 터미널 1 — 전체 (시뮬+큐브+localization+nav2+rviz+tracking+main_sim)
sim-env
ros2 launch main_sim main_sim.launch.py
...
```bash
# 좌표/방향 바꿔 실행
ros2 launch main_sim main_sim.launch.py    # (노드 파라미터는 launch에서 조정)
```
...
| 파일 | 역할 |
|------|------|
| `main_sim/main_sim_node.py` | 오케스트레이터 (상태머신) |
| `launch/main_sim.launch.py` | 시뮬+큐브브릿지+localization+nav2+rviz+tracking+main_sim 통합 |
