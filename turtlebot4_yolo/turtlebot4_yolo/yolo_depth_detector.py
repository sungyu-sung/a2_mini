"""yolo_depth_detector — RGB YOLO 탐지 + car bbox 중심 depth 측정 (단일 노드).
...
class YoloDepthDetector(Node):
    def __init__(self):
        ...
        # ---- parameters ----
        self.declare_parameter('model_path', '')   # 비우면 패키지 share의 models/best.pt
        ...
        self.get_logger().info(
            f'yolo_depth_detector 시작 — RGB:{rgb_in} + DEPTH:{depth_topic}')
        ...
        if result.boxes is not None:
            for b in result.boxes:
                ...
                if depth is None:
                    ...
                    label = f'{dist_mm / 1000.0:.2f} m' if dist_mm == dist_mm else 'no depth'
